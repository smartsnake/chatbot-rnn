...
class bot:
    ...
    def discord_main(self, args):
        model_path, config_path, vocab_path = self.get_paths(args.save_dir)
        ...
        #     response = self.discordbot(net, sess, chars, vocab, args.n, args.beam_width,
        #             args.relevance, args.temperature, args.topn, Input)
        # return response

    def sample_main(self, args):
        model_path, config_path, vocab_path = self.get_paths(args.save_dir)
        # Arguments passed to sample.py direct us to a saved model.
        # Load the separate arguments by which that model was previously trained.
        # That's saved_args. Use those to load the model.
        ...
        with tf.compat.v1.Session(config=config) as sess:
            ...
            print("Restoring weights...")
            saver.restore(sess, model_path)
            self.chatbot(net, sess, chars, vocab, args.n, args.beam_width,
                    args.relevance, args.temperature, args.topn)
            

    def initial_state(self, net, sess):
        # Return freshly initialized model states.
        return sess.run(net.zero_state)

    def forward_text(self, net, sess, states, relevance, vocab, prime_text=None):
        if prime_text is not None:
            for char in prime_text:
                if relevance > 0.:
                    ...
                    if vocab[char] == '\n':
                        states[1] = self.initial_state(net, sess)
                    else:
    ...
    def sanitize_text(self, vocab, text): # Strip out characters that are not part of the net's vocab.
        return ''.join(i for i in text if i in vocab)

    def initial_state_with_relevance_masking(self, net, sess, relevance):
        if relevance <= 0.: return self.initial_state(net, sess)
        else: return [self.initial_state(net, sess), self.initial_state(net, sess)]

    def possibly_escaped_char(self, raw_chars):
        if raw_chars[-1] == ';':
    ...
    def discordbot(self, net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, Input_Text):
        states = self.initial_state_with_relevance_masking(net, sess, relevance)
        #while True:
        user_input = Input_Text
        user_command_entered, reset, states, relevance, temperature, topn, beam_width = self.process_user_command(
            user_input, states, relevance, temperature, topn, beam_width)
        if reset: states = self.initial_state_with_relevance_masking(net, sess, relevance)
        if not user_command_entered:
            states = self.forward_text(net, sess, states, relevance, vocab, self.sanitize_text(vocab, "> " + user_input + "\n>"))
            computer_response_generator = self.beam_search_generator(sess=sess, net=net,
                initial_state=copy.deepcopy(states), initial_sample=vocab[' '],
                early_term_token=vocab['\n'], beam_width=beam_width, forward_model_fn=self.forward_with_mask,
            ...
            for i, char_token in enumerate(computer_response_generator):
                ...
                output.append(self.possibly_escaped_char(out_chars))
                #print(output, end='', flush=False)
                states = self.forward_text(net, sess, states, relevance, vocab, chars[char_token])
                if i >= max_length: break
            #print(output)
            states = self.forward_text(net, sess, states, relevance, vocab, self.sanitize_text(vocab, "\n> "))
            return output

    def chatbot(self, net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn):
        states = self.initial_state_with_relevance_masking(net, sess, relevance)
        while True:
            user_input = input('\n> ')
            user_command_entered, reset, states, relevance, temperature, topn, beam_width = self.process_user_command(
    ...
    def process_user_command(self, user_input, states, relevance, temperature, topn, beam_width):
    ...
    def scale_prediction(self, prediction, temperature):
    ...
    def forward_with_mask(self, sess, net, states, input_sample, forward_args):
        ...
        prob = prob / sum(prob)
        # Apply temperature.
        prob = self.scale_prediction(prob, temperature)
    ...
    def beam_search_generator(self, sess, net, initial_state, initial_sample,
        ...
        while True:
            ...
            l, early_term = self.consensus_length(beam_outputs, early_term_token)
            if l > 0:
